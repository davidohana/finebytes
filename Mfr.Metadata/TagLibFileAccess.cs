...
namespace Mfr.Metadata
{
    ...
    /// <summary>
    /// Shared TagLib path open and one-open tag/media snapshot mapping.
    /// </summary>
    public static class TagLibFileAccess
    {
        /// <summary>
        /// Guards that <paramref name="absolutePath"/> is an existing regular file, then opens it with TagLib.
        /// </summary>
        /// <param name="absolutePath">Fully qualified filesystem path to an existing file.</param>
        /// <returns>An open TagLib file; caller owns disposal.</returns>
        ...
        internal static TagLib.File OpenExisting(string absolutePath)
        {
            absolutePath.RequireExistingRegularFile();
            return TagLib.File.Create(new TagLib.File.LocalFileAbstraction(absolutePath));
        }
        ...
        {
            using var file = OpenExisting(absolutePath);
