...
public static class FilterEngine
{
        ...
        bool continueOnErrors)
    {
        ...
        foreach (var (file, destPath) in pending)
        {
            if (File.Exists(destPath))
                conflictDestinations.Add(destPath);
    ...
    private static string ApplySingleFilter(Filter filter, string segment, FileEntryLite file, string currentPrefix, string currentExtension)
    {
        return filter switch
        {
            ...
            TrimLeftFilter f => f.Count <= 0 ? segment : segment.Length <= f.Count ? "" : segment.Substring(f.Count),
    ...
    private sealed class TokenFormatter
    {
        ...
        private static string ResolveCounterToken(string arg, FileEntryLite file)
        {
            ...
            var value = (long)start + (long)step * n;
            var raw = value.ToString(System.Globalization.CultureInfo.InvariantCulture);
