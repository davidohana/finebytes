    ...
    public static partial class FilterEngine
    {
        ...
        private static string _ApplySingleFilter(Filter filter, string segment, FileEntryLite file, string currentPrefix, string currentExtension)
        {
            return filter switch
            {
                ...
                TrimLeftFilter f => f.Options.Count <= 0 ? segment : segment.Length <= f.Options.Count ? "" : segment[f.Options.Count..],
                TrimRightFilter f => f.Options.Count <= 0 ? segment : segment.Length <= f.Options.Count ? "" : segment[..^f.Options.Count],
                ExtractLeftFilter f => f.Options.Count <= 0 ? "" : segment.Length <= f.Options.Count ? segment : segment[..f.Options.Count],
                ExtractRightFilter f => f.Options.Count <= 0 ? "" : segment.Length <= f.Options.Count ? segment : segment[^f.Options.Count..],
