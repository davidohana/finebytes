...
namespace Mfr.Tests.Ui.FilterEditors.Trimming
{
    ...
    public sealed class ShrinkDuplicateCharactersFilterEditorViewTests
    {
        ...
        public void Shrink_duplicate_character_box_updates_chain_options()
        {
            ...
            var filter = (ShrinkDuplicateCharactersFilter)mainViewModel.FilterChainViewModel.ToChain().Steps[0].Filter;
            Assert.Equal('>', filter.Options.Character);

            box.Text = string.Empty;
            window.UpdateLayout();
            Dispatcher.UIThread.RunJobs();
            filter = (ShrinkDuplicateCharactersFilter)mainViewModel.FilterChainViewModel.ToChain().Steps[0].Filter;
