...
namespace Mfr.Tests.Architecture
{
    ...
    public sealed class ProjectReferenceArchitectureTests
    {
        ...
        /// <summary>
        /// Relative repo path → layer number (higher = closer to entry). References must target strictly lower layers.
        /// </summary>
        private static readonly Dictionary<string, int> _projectRelativePathToLayer = new(_pathComparer)
        {
            ...
            [@"Mfr.App.Ui\Mfr.App.Ui.csproj"] = 5,
        };

        private static readonly string[] _testsProjectExpectedReferences =
        [
            @"Mfr.App.Cli\Mfr.App.Cli.csproj",
            @"Mfr.App.Ui\Mfr.App.Ui.csproj",
        ];

        /// <summary>
        /// Layered stack + entry points: every reference must point at a known project in a strictly lower layer.
        /// </summary>
        [Fact]
        ...
        /// <summary>
        /// Supporting tests assembly references only the entry points, per <c>docs/mfr-folder-layering.md</c>.
        /// </summary>
        [Fact]
