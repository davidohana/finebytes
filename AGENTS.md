...
- Shared-checkout serialization: skill `mfr-queue` (`/queue`) waits until other local agents look idle before editing. Prefer `/worktree` for intentional parallel work.
- UI interaction tests: see skill `mfr-ui-headless-tests` and `Mfr.Tests/Ui/FileList/FileListViewTests.cs`.
- Plans: save all implementation / feature plans under `docs/plans/` (kebab-case names, e.g. `docs/plans/more-read-only-metadata-formats.plan.md`). Do not put project plans in `.cursor/plans/` or elsewhere.
...
- Persist **one current schema** per field (`config.json` prefs including `log` / `options` / `renameLog` / UI session sections / `filterDefaults`, presets, etc.). Use normal `[JsonPropertyName]` / record properties — not custom migration converters.
- **Soft-load vs hard-fail:** Prefs (`Mfr.Engine.Config.ConfigStore` — whole `config.json`: `log`/`options`/`renameLog`/`mainWindow`/`fileList`/`renameList`/`filterEditor`/`dialogs`/opaque `filterDefaults`) soft-load — corrupt/missing → defaults, app continues; invalid leaves skipped via `ConfigJsonApplier.ApplySoft`. Prefs **shape** (DTO records) stays in `Mfr.Models.Config`. Typed filter-default entries skip unknown payloads in `FilterDefaultsStore`. Presets (`PresetManager`) hard-fail — invalid → throw / abort load. CLI `--set` and a missing explicit `--config` path stay hard-fail. See type remarks on `ConfigStore` / `PresetManager` / `FilterDefaultsStore`.
- On soft-load prefs, missing or unrecognized fields may fall back to **defaults** (same as first launch). Do not silently remap obsolete values unless the user explicitly asks for migration. Do **not** read legacy `session.json` / `filter-defaults.json` or nested `"session"` objects.
...
Example (Rename List Auto-Sort): `renameList.sortFields` entries are `{ "key": …, "descending": … }` only — no `"column"` enum, no `SessionState*JsonConverter` for backward compatibility.
