...
First launch writes `%AppData%/finebytes/mfr/config.json` (log/ui settings, UI session layout, and per-filter add defaults) if it is missing, so those values can be edited by hand. **Tools → Reset Configuration** deletes that prefs file, skips writing session on close, and restarts the app (named `presets.json` and local logs are kept).
