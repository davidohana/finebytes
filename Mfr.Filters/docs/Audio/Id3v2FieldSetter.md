...
- **`text`** (string)
  - Plain text, **or** a formatter template when it contains at least one balanced `<...>` span that looks like a
    formatter token (same rules as [AudioTagSetter](AudioTagSetter.md) / [Formatter](../Formatting/Formatter.md)).
    Default empty (clears the frame instance).
- **`onlyIfEmpty`** (boolean)
  - When `true`, set only if the current frame text is empty/whitespace. Default `false` (overwrite).
...
- `frameId`: `TIT2`; `text`: `Fixed` — `TIT2 empty or any` → TIT2 `Fixed` — Overwrites.
- `frameId`: `TIT2`; `text`: `Fill`; `onlyIfEmpty`: `true` — TIT2 `Already` → `unchanged`
- `frameId`: `TIT2`; `text`: `Fill`; `onlyIfEmpty`: `true` — `TIT2 empty` → TIT2 `Fill`
- `frameId`: `TIT2`; `text`: `<file-name>` — prefix `Song` → TIT2 `Song` — Formatter token.
- `frameId`: `COMM`; `text`: `Hi` — `no COMM` → COMM text `Hi`, language `eng` — Primary comment.
- `frameId`: `TIT2`; `text`: `X` — `.flac` row → `preview error` — ID3v2 not supported.
