...
- `target` — for **string** filters acting on renamed metadata (most built-ins): a polymorphic object with discriminator **`targetType`** (values such as **`FilePrefix`**, **`FileExtension`**, **`FileFullName`**, **`AncestorFolder`**, **`FullPath`**, **`ParentDirectory`**, **`AudioOverlayField`**). **`AncestorFolder`** adds **`level`**: positive integer counting up from the immediate parent folder. **`AudioOverlayField`** adds **`field`**: camelCase name of the embedded audio overlay property (`title`, `album`, `year`, …); see [Formatter](Formatting/Formatter.md) **Target**.
