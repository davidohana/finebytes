...
- `delimiter`: `","`; `tokenNumber`: `2`; `moveBy`: `3`
  - Before: `milk,sugar,bread,potatoes,honey,salt,water`
  - After: `milk,bread,potatoes,honey,sugar,salt,water`
  - Comment: Move 2nd token three places right.
- `delimiter`: `","`; `tokenNumber`: `2`; `moveBy`: `3`
  - Before: `milk,sugar,bread,potatoes`
  - After: `milk,bread,potatoes,sugar`
  - Comment: Same move; target position past end → last slot.
- `delimiter`: `"-"`; `tokenNumber`: `2`; `moveBy`: `-1` — `a-b-c` → `b-a-c` — One step left: swap with preceding token.
