...
**Status (2026-09-05):** F5 Time Shifter + Attributes Setter + Audio Tag Remover done. **Next: F5 Audio Tag Setter**. Rename List Phase 10–11 already consume `ToChain()` → live preview when Auto-Preview is on.
...
- **22 (next)** — Audio Tag Setter (Audio) — per-field format specs
