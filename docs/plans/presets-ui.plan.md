...
| **Save Preset**        | One **Save Preset** entry opens a dialog with name/description/columns and footer **Update** / **Save as new** / Cancel. **Update** keeps Id (may rename when the name is free); **Save as new** upserts by name (overwrite confirm if name exists).                                                 |
| Save enabled           | **Update** enabled only when last-loaded name is still present in `NameToPreset` (cleared if deleted; updated if renamed). If none → Update disabled; Save as new still works. Both actions require a non-blank name.                                                                                |
| Save payload           | **Chain always** (`ToChain()`). Description and columns come from the dialog for both Update and Save as new (checkbox off → `null` columns).                                                                                                                                                        |
...
| Description            | Optional on Save dialog + read-only pane + **Edit Description** in Manager.                                                                                                                                                                                                                          |
| Rename preset          | **In Manager** or via **Update** in Save dialog — keep `Id`; refuse if another preset already has that exact name; update last-loaded name if it matched.                                                                                                                                            |
| Overwrite / delete     | Overwrite confirm only on **Save as new** when name exists; **confirm delete**. **Update** rename to a taken name shows an error (no overwrite).                                                                                                                                                     |
