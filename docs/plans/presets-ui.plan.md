...
| Confirm before replace | **Via `ui.suppressedConfirmations`** (`ConfirmationPolicy` / `NeedsConfirmReplaceOnLoad`). Shows until the user opts out on the dialog (or Reset confirmations in Options). See [per-dialog-confirmations.plan.md](per-dialog-confirmations.plan.md). Applies to Manager **Load** and toolbar **▾** quick-pick. |
...
flowchart LR
  ...
  cfg["ui.suppressedConfirmations"] --> confirmGate
```
...
1. **Confirm-replace gate** — was `ui.presets.confirmReplace…` bool; then `ui.confirmationPrompts`; now `ui.suppressedConfirmations` + `ConfirmationPolicy` ([per-dialog-confirmations.plan.md](per-dialog-confirmations.plan.md)).
