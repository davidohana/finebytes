...
| Confirm before replace | **Via `ui.confirmationPrompts`** (`ConfirmationPolicy` / `NeedsConfirmReplaceOnLoad`). Confirm only at **More** when the stack is non-empty; Fewer/Normal skip (same as former default-off bool). See [options-confirmations-and-double-click.plan.md](options-confirmations-and-double-click.plan.md). Applies to Manager **Load** and toolbar **▾** quick-pick. |
...
flowchart LR
  ...
  cfg["ui.confirmationPrompts"] --> confirmGate
```
...
1. **Confirm-replace gate** — was `ui.presets.confirmReplace…` bool; now `ui.confirmationPrompts` + `ConfirmationPolicy` ([options-confirmations-and-double-click.plan.md](options-confirmations-and-double-click.plan.md)).
