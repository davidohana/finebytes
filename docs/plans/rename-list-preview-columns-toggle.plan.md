...
- **Add / Set columns from filters** while A/B on: merge/replace **Original** keys only (normalize inferred Preview keys at **RL call sites** / shuttle). Do **not** change `FilterRelevantRenameListColumns` globally — that helper still returns original + preview when supported.
