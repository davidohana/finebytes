...
- Startup: keep `ResolveStartPath` fallback; when remembered path was unusable, set one **warning** status (`Last folder unavailable ({path}) — opened …`) and seed main `StatusHint` after PropertyChanged is wired (ctor status alone is missed).
