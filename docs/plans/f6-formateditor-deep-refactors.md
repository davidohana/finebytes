...
### 1. Shared labeled choice record — **high**

- **Sites:** `CounterFormatTokenEditorViewModel.PaddingChoice` / `ResetScopeChoice`; `FileDateFormatTokenEditorViewModel.KindChoice`; `FileSizeFormatTokenEditorViewModel.UnitChoice` (all `Value` + `Label` + `ToString() => Label`)
- **Target:** one shared `FormatTokenChoice(string Value, string Label)` (or similar) under FormatEditor VMs
- **Value:** delete ~4 near-identical nested records; one ComboBox row shape
- **Cost:** tiny; rename bindings in 3–4 AXAML/VM files; low risk
- **Rank:** high — do first in any polish pass

### 2. Share named-arg parse with Filters — **high**
...
### 3. One template-walk owner for Compile + TryValidate — **high**
...
### 4. Grouped insert picker when search empty — **high** (plan gap)
...
### 5. Flyout UX: focus search + Enter-to-insert — **medium**
...
### 6. Convention ViewLocator for token bodies — **medium**
...
### 7. Consistent name trimming in Compile — **medium** (product call)
...
### 8. Theme-aware error link color — **low**
...
### 9. Shared OK-only message dialog — **low**
...
### 10. Nested FormatEditor in `source=` + nested spans — **low for F6**
...
### 11. Inserter/Audio “likely-token” validation mode — **low**
