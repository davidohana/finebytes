...
2. Capture the **options body** (preferred) or the full Filter Configuration pane.
3. Save as `help/images/{Type}.png` (catalog type name).
4. Target width ~**400–600px**; HTML currently hard-codes `width="480" height="120"` — update `height` (and width if needed) to match real crops.
...
1. `just run-ui` on `:99`.
2. For each filter in inventory: add from palette → select in Filter Chain → wait for editor paint.
3. Screenshot → crop to framing choice → save `help/images/{Type}.png`.
4. Spot-check HTML in a browser (or open help file) that the figure no longer shows checkerboard.
5. Adjust each page’s `width` / `height` attributes to the real pixel size (or drop fixed height and rely on CSS `max-width`).
6. Check off rows in [`help/SCREENSHOTS.md`](../../help/SCREENSHOTS.md); remove optionless rows if we drop figures.
