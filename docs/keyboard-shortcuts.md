...
**Before/After Mode** is available on the field shuttle Columns tab and the **Rename List** menu. When on, the stored layout is originals-only; use the Rename List toolbar **Before/After** toggle (◀ / ▶) or matching **Rename List** menu radios (**Before** / **After**) to flip between original and preview values for the same fields. The side control is hidden/disabled when Before/After Mode is off. There is no dedicated keyboard shortcut in v1.
