...
Click the **Select Fields** toolbar button on the Rename List, or choose **Select Fields...** from the **Rename List** menu, the grid right-click menu, or a column-header right-click menu, to open the field shuttle (visible columns and Auto-Sort tabs). **Select Sort Fields...** on the **Rename List** menu opens the same dialog on the Sort tab. Right-click a column header and choose **Hide Field** to hide that column (persisted layout keys only — not derived A/B Preview companions). **Export Rename List (csv)...** on the **Rename List** menu (or **Export → Export Visible Columns (csv)** on a column header) writes a CSV of the on-screen columns (including derived Preview companions while A/B Preview side is showing).

**A/B Mode** is a checkbox on the field shuttle Columns tab. When on, the stored layout is originals-only; use the Rename List toolbar **Original | Preview** radios (or matching **Rename List** menu radios) to flip the grid between originals and derived preview companions. The side control is hidden/disabled when A/B Mode is off. There is no dedicated keyboard shortcut in v1.
