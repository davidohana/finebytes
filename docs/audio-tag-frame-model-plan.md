...
- [ ] **Phase 4:** **Derived semantic surface** — `Title`, `Album`, `Year`, etc. are computed from tag blocks (single source of truth); writes go through per-format mappers into frames/atoms/descriptors only; remove redundant stored façade fields and the **façade→block coalesce** step in `AudioTagPersistence.Apply` once all call sites use the mapper layer.
  - **Progress:** Preview paths merge façade edits into native blocks via **`AudioTagPersistence.TryMaterializePreviewFacadeIntoNativeBlocks`**: **`RenameList.Preview`** (end of chain), **`StringTargetFilter`** after each **`AudioOverlayField`** write, and **`AudioTagSetterFilter`** when any semantic field appears in **`options`**. **`AudioTagOverlay.TagBlocksStructurallyEquals`** and **`MergedSemanticFacadesEqual`** split block vs merged-façade comparisons. **`RenamePropertyChangeBuilder`** emits **`AudioTag.Native.*`** summary rows when structured blocks diverge even if merged scalars align. **`CommonAudioTag.FromOverlay` / `FromNativeBlocksOnly`** implement block-first read projections (façade fill-in only in **`FromOverlay`**). **Public** `MaterializePreviewFacadeIntoNativeBlocks` remains for strict call sites and tests. **StringTargetFilter** and audio format tokens read embedded fields via **`CommonAudioTag.FromOverlay`** + **`AudioOverlaySemanticFieldStrings`**. Remaining: replace stored façade properties with projections / mapper-only writes; drop redundant `Apply` coalesce when no path still relies on façade-only drift.
...
- **Phase 1 — MP3 vertical slice:** Structured `AudioTagOverlay` with **ID3v1 + ID3v2** blocks (frame inventory, deep **Clone/Equals**); [`AudioTagPersistence`](../Mfr.Metadata/AudioTagPersistence.cs) **Read/Apply for MP3 only**; **semantic properties** (`Title`, …) as projections + writers targeting ID3v2 (documented precedence vs ID3v1); update [`CommitExecutor`](../Mfr.Engine/CommitExecutor.cs) and critical tests (commit, persistence, rename list) for this path. **Outcome:** end-to-end proof without boiling the ocean.
...
- **Phase 3 — Semantic and pipeline parity:** Ensure [`AudioTagSetterFilter`](../Mfr.Filters/Audio/AudioTagSetterFilter.cs), [`FileMetaPreviewExtensions`](../Mfr.Models/FileMetaPreviewExtensions.cs), formatter tokens, and [`EmbeddedTagRemover`](../Mfr.Filters/Audio/EmbeddedTagRemoverFilter.cs) behave correctly with structured overlay; adjust [`RenamePropertyChangeBuilder`](../Mfr.Engine/RenamePropertyChangeBuilder.cs) as needed. **Outcome:** no regressions vs current feature set.
...
- Implements **`Clone`**, **`Equals`**, **`GetHashCode`** with **deep** structural equality (required by [`CommitExecutor`](../Mfr.Engine/CommitExecutor.cs), [`RenameItem.HasPreviewChanges`](../Mfr.Models/RenameItem.cs), and tests).
...
- [`RenamePropertyChangeBuilder.cs`](../Mfr.Engine/RenamePropertyChangeBuilder.cs): today it diffs flat overlay fields; either keep **semantic** lines only (simplest) or extend to show per-block/frame diffs later. After **Phase 4**, semantic rows should reflect **projected** values (or block-level deltas only).
