...
- [ ] **Phase 4:** **Derived semantic surface** — `Title`, `Album`, `Year`, etc. are computed from tag blocks (single source of truth); writes go through per-format mappers into frames/atoms/descriptors only; remove redundant stored façade fields and the **façade→block coalesce** step in `AudioTagPersistence.Apply` once all call sites use the mapper layer.
  - **Progress:** `RenameList.Preview` calls `AudioTagPersistence.TryMaterializePreviewFacadeIntoNativeBlocks` when the source path exists and is not a directory (best-effort; skips when TagLib cannot open the file as an audio container) so façade edits and per–`TagTypes` snapshots align after the filter chain. **`StringTargetFilter`** (in `Mfr.Filters`, namespace `Mfr.Models`) calls the same merge immediately after each **`AudioOverlayField`** write so later filters see updated native blocks mid-chain. **`AudioTagOverlay.TagBlocksStructurallyEquals`** exposes block-only equality for upcoming Phase 5 / deletion work. **Public** `MaterializePreviewFacadeIntoNativeBlocks` remains for strict call sites and tests. Remaining: replace stored façade properties with projections / mapper-only writes; drop redundant `Apply` coalesce when no path still relies on façade-only drift.
