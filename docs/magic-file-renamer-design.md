...
public enum LettersCaseMode { UpperCase, LowerCase, FirstLetterUp, WeirdCase, TitleCase, SentenceCase, InvertCase }
...
String-target filters are every filter derived from `StringTargetFilter` (`Mfr.Filters` namespace and assembly; case, space, trimming, replacer, formatting filters that take a `FilterTarget`). Non-string filters (`MoverFilter`, attribute/time filters, etc.) do not use `applyScope`.

`applyScope` JSON (discriminator `scopeType`, like `targetType` on `FilterTarget`):

- **`Substring`** — `startPosition`, `endPosition` (1-based, clamped to the current string length), `startAnchor`, `endAnchor` (`"left"` or `"right"`).
- **`Token`** — `separator` (non-empty), `tokenNumber` (1-based). If `tokenNumber` is greater than the number of parts after splitting, the filter is a **no-op** for that item (full string unchanged). `tokenNumber` less than 1 or an empty `separator` is invalid and fails when the scoped transform runs.
...
Omit `applyScope` (or set it to null) to transform the **entire** target string. The same optional `applyScope` shape applies to other **string-target** filters.
...
**Implementation (current):** preview tags live in `AudioTagOverlay` (parsed per–`TagTypes` blocks, not a flat `Id3Tags` bag). Commit Apply diffs session Original → Preview per block: remove dropped tag types, create new blocks, and field-patch only changed modeled frames/keys. Unmodeled content (for example APIC) survives field edits and is removed only when that whole tag type is dropped or emptied. Generic semantic writes broadcast to present blocks (or create the container’s recommended block when none exist). Format-specific filter targets (`Id3v1Field`, `Id3v2Frame`, `XiphField`) address one tag block and error on unsupported containers. Full pipeline, container policy, ID3v2/ASF rules, and known limits: [audio-tag-model.md](audio-tag-model.md).
...
> **Implementation (current)** and [audio-tag-model.md](audio-tag-model.md).
